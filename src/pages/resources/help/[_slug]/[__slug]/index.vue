...
// console.log(slug_cat); // debug
// console.log(slug); // debug
// const categoryFiltered = helpCenterCategories.find(function (data, index) {
// 	if (data.slug == slug_cat) return true;
// });
// // console.log(categoryFiltered); // debug
// const articleFiltered = categoryFiltered.articles.find(function (data, index) {
// 	if (data.slug == slug) return true;
// });

// console.log(articleFiltered); // debug
// get article data
const article = ref<HelpCenterArticle>();
async function fetchArticle() {
	try {
		article.value = await getArticle(slug, slug_cat, helpCenterCategories);
		// article.value = articleFiltered;
	} catch {
		router.replace({
			name: "all",
