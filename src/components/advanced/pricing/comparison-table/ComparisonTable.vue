		...
		<div class="comparison-table">
			<div class="comparison-table-inner">
				<div class="comparison-labels">
					<div v-for="(cell, c) in props.labels" :key="c" class="comparison-label-cell" :class="[cell.cellType && `is-cell-${cell.cellType}`]">
						<span class="mr-1">{{ cell.cellLabel }}</span>
						<Popover v-if="cell.helpText" :content="cell.helpLabel" color="primary" placement="top" arrow hover>
							<i-uil-question-circle class="text-light" />
						</Popover>
					</div>
				</div>
				<div v-for="(plan, p) in props.plans" :key="p" class="compared-plan" :class="p === 3 && 'is-premium'">
					<div class="plan-head">
					...
					<div class="plan-body">
						<div v-for="(cell, c) in plan.cells" :key="c" class="comparison-plan-cell" :class="[cell.cellType && `is-cell-${cell.cellType}`]">
							<div v-if="cell.cellType === 'label'" class="cell-inner">
								<p v-for="(label, l) in cell.cellLabel" :key="l">{{ label }}</p>
							</div>
							<div v-else-if="cell.cellType === 'check' && cell.cellCheck" class="cell-inner text-success rem-125">
