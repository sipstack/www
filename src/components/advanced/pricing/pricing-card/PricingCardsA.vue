...
const blockClasses = computed(() => [props.pulled && "is-pulled", props.rounded && "is-rounded", props.columns === 4 && "max-w-8", props.columns === 2 && "max-w-6"]);
	...
	<div class="py-6">
		<div class="pricing-cards mx-auto" :class="blockClasses">
			...
			<div class="columns b-columns-half-tablet-p">
				<div v-for="(plan, index) in props.plans" :key="index" class="column" :class="columnClasses">
					<div class="pricing-card" :class="plan.featured && 'is-featured'">
						<div class="plan-name">
							<span>{{ plan.name }}</span>
						</div>
						<div class="plan-price">
							<Title tag="h3" :size="2" weight="bold" leading :inverted="plan.featured">
								<span>{{ plan.price }}</span>
							</Title>
							<p class="paragraph" :class="plan.featured && 'is-inverted-light'">
								{{ plan.comment }}
							</p>
						</div>
