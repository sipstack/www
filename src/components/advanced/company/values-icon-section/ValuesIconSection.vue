...
const blockClasses = computed(() => [props.columns === 6 && "mx-auto max-w-7", props.columns === 4 && "mx-auto max-w-9"]);
</script>
		...
		<div class="icon-values" :class="blockClasses">
			...
			<div class="icon-values-card">
				<div class="columns b-columns-half-tablet-p">
					<div v-for="(value, index) in props.values.slice(0, props.limit)" :key="index" class="column" :class="columnClasses">
						<i class="iconify" :data-icon="value.icon" :class="`text-${value.color}`"></i>
