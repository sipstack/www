	...
	<div class="nft-marquee-wrapper">
		<MarqueeText :repeat="10" :duration="40">
			<div class="nft-marquee">
				<RouterLink v-for="(item, index) in props.items" :key="index" to="/" class="nft-marquee-item" :class="`is-${item.size}`">
					<img :src="item.preview" alt="marquee image" width="600" height="510" @error.once="(event) => viaPlaceholderErrorHandler(event, `600x510`)" />
				</RouterLink>
			</div>
		</MarqueeText>
	</div>
