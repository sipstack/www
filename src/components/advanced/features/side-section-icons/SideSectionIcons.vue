...
const sideSectionClasses = computed(() => [props.inverted && `is-inverted`, props.reverse && `is-reverse`]);

const textClasses = computed(() => [props.inverted && `is-inverted`]);
</script>

<template>
	<div class="side-section mobile:py-0 small:py-6 z-1" :class="sideSectionClasses">
		<div class="columns is-vcentered b-columns-half-tablet-p">
			<div class="column is-6">
				<div class="max-w-5">
					...
					<Title tag="h2" :size="3" weight="semi" :inverted="props.inverted" leading>
						{{ title }}
					</Title>
					<slot name="content">
					...
					<slot name="features">
						<div class="columns is-multiline b-columns-half-tablet-p">
							<div v-for="(feature, index) in props.features.slice(0, 4)" :key="index" class="column is-6">
								<div class="py-2 medium:py-3">
									<i class="iconify" :data-icon="feature.icon"></i>
									<Title tag="h3" :size="6" weight="semi" leading :inverted="props.inverted">
			...
			<div class="column is-6 has-text-centered is-relative">
				...
				<DarkImage class="is-relative mx-auto" :src="props.image" :src-dark="props.darkImage" alt="side section image" :width="props.imageWidth" :height="props.imageHeight" />
			</div>
		</div>
	</div>
</template>
