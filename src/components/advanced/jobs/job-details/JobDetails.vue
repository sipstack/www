		...
		<div class="job-header">
			<div class="left">
				...
				<div class="meta">
					...
					<h4 v-if="props.job.location || props.job.duration" class="job-subtitle">
						<i class="iconify" aria-hidden="true" data-icon="feather:map-pin"></i>
						{{ props.job.location }} -
		...
		<div class="job-body">
			<div class="columns">
				<div class="column is-7">
					<template v-for="(content, descKey) in props.job.content.description" :key="descKey">
						<h3>{{ content.title }}</h3>
						<Markdown :source="content.text" />
					</template>

					<div v-if="props.job.content.aptitude" class="requirements-list">
						<template v-for="(content, aptKey) in props.job.content.aptitude" :key="aptKey">
							<h3>{{ content.title }}</h3>
				...
				<div class="column is-4 is-offset-1">
					<div v-if="props.job.content.technologies" class="recommended-skills">
						<template v-for="(content, techKey) in props.job.content.technologies" :key="techKey">
							<h3>{{ content.title }}</h3>

							<div v-for="(skill, toolKey) in content.tools" :key="toolKey" :class="'skill-block'">
