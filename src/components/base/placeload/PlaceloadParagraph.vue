...
if (props.width.match(CssUnitRe) === null) {
	console.warn(`PlaceloadParapgraph: invalid "${props.width}" width. Should be a valid css unit value.`);
}
if (props.lastLineWidth.match(CssUnitRe) === null) {
	console.warn(`PlaceloadParapgraph: invalid "${props.lastLineWidth}" lastLineWidth. Should be a valid css unit value.`);
}
</script>

<template>
	<div class="content-shape-group">
		<PlaceloadText v-for="line of props.lines - 1" :key="line" :width="props.width" :centered="props.centered" />
