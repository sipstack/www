...
const avatarGroupClasses = computed(() => [props.compact ? `is-compact` : `is-loose`, props.boxed && "is-boxed", props.squared && `is-squared`]);

const avatarCounterClasses = computed(() => [props.size && `is-${props.size}`, props.squared && `is-squared`]);
...
<template>
	<div class="avatar-group" :class="avatarGroupClasses">
		<AvatarSimple v-for="(avatar, index) in props.avatars.slice(0, props.limit)" :key="index" :picture="avatar.picture" :squared="props.squared" :size="props.size" />
		<div v-if="props.counter && props.limit < props.avatars.length" class="avatar-counter" :class="avatarCounterClasses">
