...
export type ToggleColor = "default" | "primary" | "success" | "info" | "warning" | "danger";
...
const toggleClasses = computed(() => [props.flavor && `is-${props.flavor}`, props.color && `is-${props.color}`]);
</script>

<template>
	<label :for="`toggle-${props.name}`" class="ninja-toggle" :class="toggleClasses">
		<slot></slot>

		<input :id="`toggle-${props.name}`" :value="props.value" :checked="checked" type="checkbox" @change="change" />
		<span class="no-margin">
			<span class="track">
				<span class="track-inner"></span>
				<span class="track-knob">
					<i v-if="props.icon" class="inactive iconify" data-icon="feather:circle"></i>
					<i v-if="props.icon" class="active iconify" data-icon="feather:check"></i>
				</span>
			</span>
		</span>
	</label>
