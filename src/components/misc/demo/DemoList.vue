...
function useFilter(items: DemoItem[], filter: Ref<string>): ComputedRef<DemoItem[]> {
	return computed(() => {
		...
		return items.filter((demo) => {
			return demo.name.match(filterRe) || demo.category.match(filterRe) || demo.keywords?.match(filterRe) || demo.route.name.replace(/-/g, " ").match(filterRe);
		});
	});
}
...
const totalDemos = landingDemos.length + aboutDemos.length + pricingDemos.length + blogDemos.length + subDemos.length + authDemos.length + layoutsDemos.length;
	...
	<div class="vulk-demos">
			...
			<div class="search-field-wrapper mx-auto max-w-3">
				<Field :label="searchLabel">
					<Control icon="feather:search">
						<input v-model="filterInput" type="search" class="input is-rounded" placeholder="Banking, Startup, Freelancer ..." />
					</Control>
				</Field>
			</div>
		</div>
		<div v-if="totalResults === 0">
          ...
          criteria."
			...
			>
				<template #image>
					<DarkImage src="/assets/illustrations/placeholder/not-found.svg" src-dark="/assets/illustrations/placeholder/not-found-dark.svg" alt="Placeholder image" />
				</template>

				<template #action>
					<Button :href="generateFeatureRequestLink(filterInput)" target="_blank" icon-left="feather:github"> Request a Feature </Button>
				</template>
			</Placeholder>
		</div>
		...
		<div v-if="landingDemosFiltered.length > 0" class="demo-section">
			<div class="demo-section-title">
				<DarkImage src="/assets/icons/layouts/layout-1.svg" src-dark="/assets/icons/layouts/layout-1-dark.svg" alt="Placeholder image" />
			...
			<div class="columns is-multiline">
				<!-- Demo item -->
				<div v-for="(item, index) in landingDemosFiltered" :key="index" class="column is-4 has-text-centered">
					<figure class="vulk-demo-wrapper loaded" tabindex="0" @keydown.space.prevent="(e) => ((e.target as HTMLElement).querySelector('a[href]') as HTMLAnchorElement)?.click()">
						<span v-if="item.new" class="new-tag">New</span>
						<img v-if="!darkmode.isDark" :src="item.screenshot.light" alt="Demo screenshot" loading="lazy" />
						<img v-else :src="item.screenshot.dark" alt="Demo screenshot" loading="lazy" />
		...
		<div v-if="aboutDemosFiltered.length > 0" class="demo-section">
			...
			<div class="columns is-multiline">
				<!-- Demo item -->
				<div v-for="(item, index) in aboutDemosFiltered" :key="index" class="column is-4 has-text-centered">
					<figure class="vulk-demo-wrapper loaded" tabindex="0" @keydown.space.prevent="(e) => ((e.target as HTMLElement).querySelector('a[href]') as HTMLAnchorElement)?.click()">
						<span v-if="item.new" class="new-tag">New</span>
						<img v-if="!darkmode.isDark" :src="item.screenshot.light" alt="Demo screenshot" loading="lazy" />
						<img v-else :src="item.screenshot.dark" alt="Demo screenshot" loading="lazy" />
		...
		<div v-if="pricingDemosFiltered.length > 0" class="demo-section">
			...
			<div class="columns is-multiline">
				<!-- Demo item -->
				<div v-for="(item, index) in pricingDemosFiltered" :key="index" class="column is-4 has-text-centered">
					<figure class="vulk-demo-wrapper loaded" tabindex="0" @keydown.space.prevent="(e) => ((e.target as HTMLElement).querySelector('a[href]') as HTMLAnchorElement)?.click()">
						<span v-if="item.new" class="new-tag">New</span>
						<img v-if="!darkmode.isDark" :src="item.screenshot.light" alt="Demo screenshot" loading="lazy" />
						<img v-else :src="item.screenshot.dark" alt="Demo screenshot" loading="lazy" />
		...
		<div v-if="blogDemosFiltered.length > 0" class="demo-section">
			...
			<div class="columns is-multiline">
				<!-- Demo item -->
				<div v-for="(item, index) in blogDemosFiltered" :key="index" class="column is-4 has-text-centered">
					<figure class="vulk-demo-wrapper loaded" tabindex="0" @keydown.space.prevent="(e) => ((e.target as HTMLElement).querySelector('a[href]') as HTMLAnchorElement)?.click()">
						<span v-if="item.new" class="new-tag">New</span>
						<img v-if="!darkmode.isDark" :src="item.screenshot.light" alt="Demo screenshot" loading="lazy" />
						<img v-else :src="item.screenshot.dark" alt="Demo screenshot" loading="lazy" />
		...
		<div v-if="subDemosFiltered.length > 0" class="demo-section">
			...
			<div class="columns is-multiline">
				<!-- Demo item -->
				<div v-for="(item, index) in subDemosFiltered" :key="index" class="column is-4 has-text-centered">
					<figure class="vulk-demo-wrapper loaded" tabindex="0" @keydown.space.prevent="(e) => ((e.target as HTMLElement).querySelector('a[href]') as HTMLAnchorElement)?.click()">
						<span v-if="item.new" class="new-tag">New</span>
						<img v-if="!darkmode.isDark" :src="item.screenshot.light" alt="Demo screenshot" loading="lazy" />
						<img v-else :src="item.screenshot.dark" alt="Demo screenshot" loading="lazy" />
		...
		<div v-if="authDemosFiltered.length > 0" class="demo-section">
			...
			<div class="columns is-multiline">
				<!-- Demo item -->
				<div v-for="(item, index) in authDemosFiltered" :key="index" class="column is-4 has-text-centered">
					<figure class="vulk-demo-wrapper loaded" tabindex="0" @keydown.space.prevent="(e) => ((e.target as HTMLElement).querySelector('a[href]') as HTMLAnchorElement)?.click()">
						<span v-if="item.new" class="new-tag">New</span>
						<img v-if="!darkmode.isDark" :src="item.screenshot.light" alt="Demo screenshot" loading="lazy" />
						<img v-else :src="item.screenshot.dark" alt="Demo screenshot" loading="lazy" />
		...
		<div v-if="authDemosFiltered.length > 0" class="demo-section">
			...
			<div class="columns is-multiline">
				<!-- Demo item -->
				<div v-for="(item, index) in layoutsDemosFiltered" :key="index" class="column is-4 has-text-centered">
					<figure class="vulk-demo-wrapper loaded" tabindex="0" @keydown.space.prevent="(e) => ((e.target as HTMLElement).querySelector('a[href]') as HTMLAnchorElement)?.click()">
						<span v-if="item.new" class="new-tag">New</span>
						<img v-if="!darkmode.isDark" :src="item.screenshot.light" alt="Demo screenshot" loading="lazy" />
						<img v-else :src="item.screenshot.dark" alt="Demo screenshot" loading="lazy" />
