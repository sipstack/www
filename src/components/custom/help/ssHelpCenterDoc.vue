...
export interface HelpCenterDocProps {
  content?: string
  sections?: Array[]
}

const props = withDefaults(defineProps<HelpCenterDocProps>(), {
  content: '',
  section: [],
})
...
<template>
  <div>
    <div v-if="props.content" class="vulk-docs">
      <div class="left">
        <div class="left-menu">
          <ul overflown>
            <li
              ...
              <a
                ...
                @click.prevent="
                  scrollTo(
                    `#${group.name.replace(/ /g, '-').toLowerCase()}`,
                    800,
                    {
                      offset: -100,
                    }
    ...
    <div v-else class="vulk-docs">
      Our developers & technical writers are hard at work getting our
      documentation updates... Please check back later!
    </div>
  </div>
</template>
