...
const props = withDefaults(defineProps<PricingSoloProps>(), {
	monthlyTab: "Monthly Billing",
	yearlyTab: "Anual Billing",
	monthlyPer: "per member, per month",
	yearlyPer: "per member, per year",
});
		...
		<div class="pricing-solo-head">
			<div class="billing-selector">
				<div class="billing-selector-inner">
					<button :class="activePricing === 'monthly' && 'is-active'" @click="activePricing = 'monthly'">{{ props.monthlyTab }}</button>
					<button :class="activePricing === 'yearly' && 'is-active'" @click="activePricing = 'yearly'">{{ props.yearlyTab }}</button>
					<div class="naver"></div>
				</div>
			</div>
		</div>
			...
			<div class="pricing-solo-card">
				<div class="left">
					...
					<div class="inner-features">
						<div v-for="(feature, index) in props.features.slice(0, 10)" :key="index" class="feature-item">
							<span>{{ feature }}</span>
						</div>
					</div>
				</div>
				<div class="right">
					<div>
						<div v-if="activePricing === 'monthly'" class="price">
							...
							<p class="paragraph rem-95">{{ props.monthlyPer }}</p>
						</div>
						<div v-else class="price">
							<div class="price-value">
								<span>{{ props.yearlyPrice }}</span>
							</div>
							<p class="paragraph rem-95">{{ props.yearlyPer }}</p>
						</div>
						<div class="action">
