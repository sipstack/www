...
const boxClasses = computed(() => [props.animated && `animated`, !props.horizontal && `has-text-centered p-6 large:p-10`]);

const imageClasses = computed(() => [props.size && `is-${props.size}`]);

const textClasses = computed(() => [props.size === "small" && "rem-90", props.links && `mb-4`, !props.horizontal && `mx-auto max-w-2`]);
</script>

<template>
	<div class="py-6 mx-auto max-w-9">
		<div class="columns is-multiline b-columns-half-tablet-p">
			<div v-for="(feature, index) in props.features.slice(0, props.limit)" :key="index" class="column" :class="columnClasses">
				<div class="box" :class="boxClasses">
					<slot name="column" :feature="feature" :index="index">
						<DarkImage :src="feature.image" :src-dark="feature.darkImage" :class="['feature-block-image', ...imageClasses]" alt="featured image" />
						<Title tag="h3" :size="6" weight="semi" leading>
							<span>{{ feature.title }}</span>
						</Title>
						<p class="paragraph rem-95">{{ feature.text }}</p>
						<RouterLink v-if="props.links" class="action" :class="textClasses" :to="feature.link">
							<span>Read More</span>
