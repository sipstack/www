...
const blockClasses = computed(() => [props.squared && `is-squared`, props.grayscale && `is-grayscale`]);
		...
		<div class="columns is-multiline b-columns-third-tablet-p">
			<div v-for="(item, index) in props.items.slice(0, props.limit)" :key="index" class="column" :class="columnClasses">
				<div class="team-block-item mb-8">
					...
					<img class="block" :src="item.image" alt="featured image" width="270" height="333" @error.once="(event) => viaPlaceholderErrorHandler(event, `270x333`)" />
					<div>
					...
					<div class="is-flex is-justify-content-center">
						<RouterLink v-for="(link, i) in item.socialLinks" :key="i" :to="link.url" target="_blank" class="px-2">
