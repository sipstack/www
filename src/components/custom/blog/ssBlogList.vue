...
const blockClasses = computed(() => [props.singleList && !props.sideList && "max-w-7"]);
...
<template>
	<div class="blog-grid mx-auto" :class="blockClasses">
		<div class="columns b-columns-half-tablet-p is-multiline">
			<div v-for="(post, index) in props.posts.slice(0, props.limit)" :key="index" class="column" :class="columnClasses">
				<ssBlogListItem :post="post" :upside-down="props.upsideDown" />
			</div>
		</div>
	</div>
</template>
