	...
	<div class="py-6">
		<div class="columns is-multiline is-vcentered">
			<div class="column is-5">
				<slot name="content">
					<Title tag="h2" :size="2" weight="semi" leading :inverted="props.inverted">
						<span>{{ props.title }}</span>
					</Title>
					<p class="paragraph max-w-tp-full medium:max-w-4 mb-4" :class="textClasses">
						{{ props.content }}
					</p>
				</slot>
			</div>
				...
				<div class="columns is-multiline b-columns-half-tablet-p">
					<div v-for="(feature, index) in props.features.slice(0, 4)" :key="index" class="column is-6">
						<div class="py-2 medium:py-4">
							...
							<ActionLink v-if="props.links" :link="feature.link" label="Read More" size="small" />
						</div>
					</div>
				</div>
			</div>
