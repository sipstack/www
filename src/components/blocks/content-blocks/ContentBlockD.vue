	...
	<div class="py-12 mx-auto max-w-9">
		...
		<slot name="images">
			<div class="is-flex b-only-mobile-flex-wrap">
				<div v-for="(image, index) in props.images.slice(0, props.limit)" :key="index" class="max-w-mobile px-2" :class="wrapperClasses">
					<img class="inline-block mx-2" :class="imageClasses" :src="image.url" :alt="image.label" />
				</div>
			</div>
		</slot>
			...
			<slot name="content">
				<div class="columns">
					<div class="column">
						...
						<ActionLink v-if="props.link" :label="props.linkLabel" :link="props.link" />
					</div>
				</div>
			</slot>
		</div>
