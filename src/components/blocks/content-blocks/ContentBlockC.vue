...
<template>
	<div class="py-6 mx-auto max-w-6">
		...
		<slot name="image">
			<div>
				<img class="inline-block" :class="imageClasses" :src="props.image" alt="featured image" />
			</div>
		</slot>
		...
		<slot name="content">
			<div class="columns b-columns-half-tablet-p py-4">
				<div v-for="(content, index) in props.text.slice(0, 2)" :key="index" class="column is-6">
					...
					<ActionLink v-if="props.links" :label="props.linksLabel" :link="content.link" size="small" />
				</div>
			</div>
		</slot>
	</div>
