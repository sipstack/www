	...
	<div class="py-6 mx-auto max-w-8">
		<slot name="title">
			<div class="has-text-centered mb-8">
				<SectionTitle :title="props.title" :subtitle="props.subtitle" :inverted="props.inverted" />
			</div>
		</slot>
		<div class="columns m-0">
			<slot name="content">
				<div v-for="(content, index) in props.text.slice(0, 2)" :key="index" class="column is-6">
		...
		<div class="mx-auto max-w-6 is-flex is-justify-content-center pt-8">
			<slot name="images">
				<div v-for="(image, index) in props.images.slice(0, 5)" :key="index" class="image-wrap" :class="imageClasses">
					<ClientOnly>
						<Popper placement="top" arrow hover offset-distance="20">
							<AvatarSimple :picture="image.url" :squared="props.squared" size="large" />
							<template #content>
								<div class="popover-head">
									<AvatarSimple :picture="image.url" :squared="props.squared" size="small" />
									<div class="meta">
		...
		<div v-if="props.link" class="bottom-action is-flex is-align-items-center is-justify-content-center pt-8">
