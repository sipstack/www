...
const blockClasses = computed(() => [props.grayscale && `is-grayscale`, props.scale && `is-scale`, props.plain && `is-plain`, props.plain && props.curved && `is-curved`]);
...
<template>
	<div class="py-6">
		<div class="columns is-multiline b-columns-half-tablet-p" :class="blockClasses">
			<div v-for="(item, index) in props.items.slice(0, props.limit)" :key="index" class="column" :class="columnClasses">
				<div class="card">
					<div class="card-image">
						<img :src="item.image" alt="Team member image" @error.once="(event) => viaPlaceholderErrorHandler(event, '500x333')" />
					</div>
					<div class="card-content">
						<div class="media is-align-items-center">
							...
							<div class="media-right">
								<RouterLink v-for="(link, i) in item.socialLinks" :key="i" :to="link.url" target="_blank" class="px-1">
