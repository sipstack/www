...
const galleryClasses = computed(() => [props.gap && `gap-${props.gap}`, props.squared && `is-squared`]);
...
<template>
	<div class="gallery py-6" :class="galleryClasses">
		<div class="columns is-multiline b-columns-half-tablet-p">
			<div v-for="(item, i) in props.items.slice(0, props.limit)" :key="i" class="column" :class="columnClasses">
				<img class="block" :src="item.image" alt="featured image" @click.prevent="showLightbox(item.image)" @keydown.space.prevent="() => showLightbox(item.image)" />
			</div>
		</div>
		<teleport to="body">
			<lightbox
